...
function hiddenPresetError(id: string): RemoteError {
  return new RemoteError(
    'agent-preset/invalid',
    `agent-presets: preset "${id}" is not available on Windows`,
    { agentPreset: id, reason: 'hidden on Windows' },
  )
}
...
export class WindowsAgentPresets extends AgentPresets {
  ...
  override async copy(from: string, id: string, name?: string): Promise<void> {
    if (id === WINDOWS_UNSUPPORTED_PRESET) throw hiddenPresetError(id)
